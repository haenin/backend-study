...
public class FileUploadController {
    ...
    /* 설명. application.yml에서 파일업로드 공통 경로를 불러와
    *       변수에 대입*/
    @Value("${filepath}")
                             ...
                             RedirectAttributes rttr)
    {
        ...
        *       rename해보기 */
        ...
        try {
            ...
            /* 설명. 이후 service 게층을 통해 DB에 사용자가 업로드한
            *       하나의 파일에 대한 내용을 저장하고 옴. */
    ...
    public String multiFileUpload(@RequestParam List<MultipartFile> multiFiles,
        ...
        *       문자열을 담은 컬렉션 */
        ...
        try {
            ...
            } // 두개는 성공했으나 세번째를 실패했을 수 도 있어서 전체를 감쌈
            ...
            for (int i = 0; i < files.size(); i++) { // 업로드에 성공한 것들은 List에 쌓여다는 생각으로
                Map<String, String> file = files.get(i);
                new File(filepath + "/img/multi/" + file.get("saveName")).delete();
            }
            rttr.addFlashAttribute("message","다중 파일 업로도 실패!");
        }
        return "redirect:/result";
    }
}
