package com.haenin.section03.primarykey.subsection01.identity;
...
public class PrimaryKeyMappingTest {
    ...
    public void initManager() {
        /* 설명. EntityManager가 생성될 때마다
            고유의 새로운 영속성 컨텍스트(Entity 객체를 관리하는 창고)가 생성된다. */
    ...
    public void 식별자_매핑_테스트() {
        ...
        *       즉시 flush() 호출됨 */
        ...
        /* 설명. persist 당시에는 부여되지 않은 pk값으로
         *       commit 이후 조회를 하면 가능할까? */

        Member selectedMember = entityManager.find(Member.class, 1);
        System.out.println("selectedMember = " + selectedMember);

        entityTransaction.commit();
        /* 설명. insert 전에 영속상태의 엔티티 객체에는 pk값이 없었지만
        *       persist이후 find 시에는 존재 */
